$ScriptBlockHash = [ordered]@{}
$ScriptBlockHash.'Get-UDFDriveStats'  += @{
    BlockName   = 'Get-UDFDriveStats'
    Description = "
        Returns hostname, drivename, label, total space, free space, used space, free space in percent.
    "
    ExecCmd     = {&($ScriptBlockHash.'Get-UDFDriveStats').Scriptblock.command -Filter $Filter -labels $Labels}
    ScriptBlock = @{
        Command         = {
...
$ScriptBlockHash.'Get-UDFHostNames'   += @{
    BlockName   = 'Get-UDFHostNames'
    ExecCmd     = {&($ScriptBlockHash.'Get-UDFHostNames').Scriptblock.command -HostName $HostNameFilter}
    ScriptBlock = @{
        Command         = {
    param(
        [Parameter(Position = 0,Mandatory = $true)]
...
$ScriptBlockHash.'Test-UDFConnection' += @{
    BlockName   = 'Test-UDFConnection'
    ExecCmd     = {start-job -Name $_ -ScriptBlock $ScriptBlockHash.'Test-UDFConnection'.ScriptBlock.Command -ArgumentList $_}
    ScriptBlock = @{
        Command         = {
    param(
...
$HostNameFilter = '**'
$results        = &$ScriptBlockHash['get-udfhostnames'].ExecCmd
$results

$results.HostNames | foreach{&($ScriptBlockHash.'Test-UDFConnection'.ExecCmd) -HostName $_ }
...
$issessions | foreach {Invoke-Command -Session $_ -ScriptBlock $ScriptBlockHash.'Get-UDFDriveStats'.ScriptBlock.Command -ArgumentList $Filter,$Labels -AsJob}
$sessionresults = Get-Job | Receive-Job | Select-Object Systemname,Name,Label,Capacity_gb,FreeSpace_gb,UsedSpace_gb,Free_Per
$sessionresults +=  &$scriptblockhash.'Get-UDFDriveStats'.ExecCmd
...
#$html | clip.exe
$hmtl | Out-File C:\Temp\psdrives.html

 Send-MailMessage -BodyAsHtml $html -Subject "PowerShell: Test Storage Report" -From 'SMTP' -SmtpServer
